...
| **Config** | `.env.example` · `.nvmrc` (Node 20) · `next.config.ts` (en-têtes sécurité, **HSTS** si `VERCEL_ENV=production`) · `vercel.json` (cron **1×/jour** `GET /api/cron/meal-reminders` pour plan Vercel **Hobby** ; rappels J-24/J-2h + clôture auto, `CRON_SECRET` sur Vercel ; cron **horaire** possible en **Pro** — voir `docs/DEPLOIEMENT_VERCEL.md`) · `middleware.ts` (session Supabase ; hors statiques, métadonnées crawl & **`/api/cron/*`**) · `robots.ts` / `sitemap.ts` (SEO ; désactivés en bêta publique) · `not-found` / `error` / `global-error` · `/api/health` + `version` (`src/lib/app-version.ts`) · CI `.github/workflows/ci.yml` (job `workflow-lint` fail-fast, puis lint/types/build + **Playwright** desktop, **mobile-consistency**, **bêta SEO**) + `.github/workflows/ci-governance.yml` (garde-fou workflow/docs) + `.github/workflows/nightly-release-gate.yml` (gate strict nocturne `verify:release`) + `.github/workflows/meal-reminders-cron.yml` (**cron horaire** côté GitHub si Hobby — secrets `CRON_MEAL_REMINDERS_BASE_URL` + `CRON_SECRET`, voir `docs/DEPLOIEMENT_VERCEL.md`) |
| **DB** | Migrations dans `supabase/migrations/` : noyau `20260412120000_core_profiles_meals.sql` + **Graille+** `20260418140000_graille_plus_share_rescue_payments.sql` (partage, seconde graille, `payment_ledger`) + instrumentation growth `20260420100000_growth_events.sql` (`growth_events`) + vues KPI `20260420103000_growth_kpi_views.sql` → `20260426120000_growth_kpi_partners.sql` → **`20260427100000_growth_kpi_funnel.sql`** → **`20260502100000_growth_kpi_funnel_invite_feedback.sql`** → **`20260526120000_growth_kpi_surprise_graille.sql`** (`growth_kpi_daily` : funnel + surprise graille + attributions invitation + événements feedback) + préférences notif `20260416093000_user_settings_notification_prefs.sql` + compteur e-mail jour `20260428100000_user_settings_email_nudge_counter.sql` + RPC atomique `20260429103000_reserve_meal_email_nudge_rpc.sql` (`user_settings`) + rappels repas `20260430100000_meals_reminder_columns.sql` + clôture auto `20260430200000_auto_complete_meals_rpc.sql` (`meals`) + tables / vues notifs et feedback (`user_notifications`, `user_feedback`, …). **Ordre** : appliquer **toutes** les migrations du dossier sur la base distante après chaque release qui touche le schéma. |
...
| `npm run smoke:public` | Smoke HTTP des routes publiques + invariants HTML (`validateHomeHtml` : accueil kicker + **`data-ptg-hero-raster`** / **`ptg-home-cinematic-band`** + **`ptg-home-market-band`** si raster actif + `og:image` ; bandeau `ptg-night-stage` sur Partenaires / Expériences / Repas ouverts) — serveur requis. |
