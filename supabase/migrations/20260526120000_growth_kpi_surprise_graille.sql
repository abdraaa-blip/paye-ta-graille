-- Colonne funnel : tirages « Lancer la graille » (événement surprise_graille_rolled).
...
select
  ...
  count(*) filter (where event_name = 'surprise_graille_rolled') as funnel_surprise_graille_rolled,
  ...
  count(*) filter (where event_name = 'invite_attribution') as funnel_invite_attributions,
  count(*) filter (where event_name = 'feedback_submitted') as funnel_feedback_submitted,
...
comment on view public.growth_kpi_daily is
  'Agrégats journaliers growth: funnel produit, surprise graille, invitations attribuées, feedback, partenaires, actifs.';
