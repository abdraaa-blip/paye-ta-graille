...
const querySchema = z.object({
  place_id: z.string().min(4).max(256),
});
...
export async function GET(request: Request) {
  ...
  const limited = await rateLimitForUser(session.user.id, "places_details", 48, 60_000);
  if (limited) return limited;

  const url = new URL(request.url);
  const parsed = querySchema.safeParse({ place_id: url.searchParams.get("place_id") ?? "" });
  if (!parsed.success) {
    return jsonError("validation_error", "Paramètre place_id invalide.", 400);
  }
  ...
  gUrl.searchParams.set("place_id", parsed.data.place_id);
  gUrl.searchParams.set("fields", "place_id,name,formatted_address,geometry");
