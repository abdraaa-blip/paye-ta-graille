...
| Fichier | Rôle |
|---------|--------|
| `vercel.json` | **Cron** : `GET /api/cron/meal-reminders` **1× par jour** à **07:30 UTC** (compatible **plan Hobby** ; rappels J-24 / J-2h idéalement avec un appel **horaire** — voir § Cron ci-dessous). Rewrites restent optionnels selon besoin |
...
- **Cron (rappels repas J-24 / J-2h + clôture auto)** : `vercel.json` déclenche **`GET /api/cron/meal-reminders` une fois par jour** (`30 7 * * *` → 07:30 UTC), limite **plan Vercel Hobby** (« au plus une fois par jour »). La route gère quand même les repas dans les fenêtres **si** l’appel tombe au bon moment ; pour des rappels **fiables** J-24 / J-2h, prévoir soit le **plan Pro** (cron horaire possible, ex. `0 * * * *`), soit un **scheduler externe** (GitHub Actions `schedule`, Upstash QStash, etc.) qui appelle la même URL avec `Authorization: Bearer <CRON_SECRET>`. Après les rappels : repas **`confirmed`** dont la fin de créneau + **`PTG_MEAL_AUTO_COMPLETE_GRACE_HOURS`** (défaut 24) est dépassée → **`completed`** (sauf **`PTG_MEAL_AUTO_COMPLETE=off`**). Sur Vercel : **`CRON_SECRET`** (Bearer). Sans secret → **503**. Mails : **`RESEND_API_KEY`** ; données repas : **`SUPABASE_SERVICE_ROLE_KEY`** ; migrations `20260430100000_meals_reminder_columns.sql` et `20260430200000_auto_complete_meals_rpc.sql`.
- **GitHub Actions (rappels horaires, recommandé avec Hobby)** : le workflow **`.github/workflows/meal-reminders-cron.yml`** envoie un **GET** vers `{origine}/api/cron/meal-reminders` **chaque heure** (cron `0 * * * *` UTC), avec `{origine}` = secret **`CRON_MEAL_REMINDERS_BASE_URL`** (sans slash final, ex. `https://paye-ta-graille.vercel.app`). Secrets dépôt GitHub → **Settings → Secrets and variables → Actions** : **`CRON_MEAL_REMINDERS_BASE_URL`**, **`CRON_SECRET`** (identique à Vercel). Si l’un des deux manque, le job affiche un **avertissement** et quitte sans erreur. Déclenchement manuel : **Actions → Meal reminders cron (hourly) → Run workflow**. Si le `curl` **échoue** après secrets présents, l’exécution publie l’artefact **`meal-reminders-cron-response`** (corps de réponse enregistré) pour diagnostic.
