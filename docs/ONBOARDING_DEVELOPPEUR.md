...
**Bande ciné accueil** (`HomeCinematicMarqueBand`) : barre d’outils **`role="toolbar"`** en **deux zones** — à gauche **lecture** (« Passer l’animation » pendant l’intro, puis « Rejouer l’intro ») ; à droite **effets** (« Pause les effets » / « Reprendre… », fige GSAP + halage + brume + balayage). Sur **cadre étroit** ou **viewport** réduit, libellés **courts** à l’écran + **`aria-label`** avec la phrase complète. **Pause** mémorisée dans **`sessionStorage`** (`ptg_home_cinematic_motion_pause`). `prefers-reduced-motion` : pas d’animation ni de barre d’outils ciné. **Rejeu auto** (intro + accroches) : défaut **60 s** via `cinematicAutoReplayIntervalSec()` (`src/lib/env-public.ts`) ; **`NEXT_PUBLIC_PTG_CINEMATIC_AUTO_REPLAY_SEC`** (min 30, max 600) — désactiver avec **`0`** / **`false`** / **`off`** / **`no`** ; ignoré si pause, onglet masqué, ou réduction des animations. Parser partagé : `scripts/lib/cinematic-auto-replay-interval.mjs`, `scripts/__tests__/cinematic-auto-replay-interval.test.mjs`.
